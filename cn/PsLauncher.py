...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        sys_menu.addSeparator()
        self.auto_minimize_action = QAction("启动时自动最小化到托盘", self)
        ...
        # 自动换行切换菜单项
        self.toggle_wrap_action = QAction("切换自动换行模式", self)
        ...
        # 语法着色方式子菜单
        syntax_menu = view_menu.addMenu("语法着色方式")

        # 自动模式
        self.syntax_auto_action = QAction("自动（按照后缀选择）", self)
        ...
        # 不着色模式
        self.syntax_none_action = QAction("不进行着色", self)
        ...
        if syntax_mode == 'auto':
            ...
            raise ValueError("怎么可能？？不应该运行到这里的。")
    ...
    def save_config(self):
        ...
        if missing_in_supported:
            warnings.append(f"支持的后缀列表中缺少以下默认后缀：{', '.join(missing_in_supported)}")
        if missing_in_runnable:
            warnings.append(f"可运行的后缀列表中缺少以下默认后缀：{', '.join(missing_in_runnable)}")

        if warnings:
            warning_message = "警告：\n" + "\n".join(warnings)
            warning_message += "\n\n无法为你保存当前配置。这可能导致某些脚本无法正确显示或运行。"

            # 直接弹窗显示警告并立即返回，不进行任何修改
            QMessageBox.warning(self, "配置警告", warning_message)
        else:
    ...
    def run_selected_script(self, script_path=None):
        ...
        if script_path:
            if os.path.isfile(script_path):
                ...
                if ext in self.config.get('runnable_extensions', DEFAULT_EXT):
                    ...
                    QMessageBox.information(self, "失败", f"脚本文件 '{os.path.basename(script_path)}' 的后缀 {ext} 不在可运行的后缀列表中。", QMessageBox.Ok)
            else:
                QMessageBox.information(self, "失败", "选择的路径不是一个有效的文件。", QMessageBox.Ok)
            return
        ...
        if script_path and os.path.isfile(script_path):
            ...
            QMessageBox.information(self, "失败", "无法获取有效的脚本路径。", QMessageBox.Ok)
    ...
    def new_script_in_folder(self):
        ...
        file_name, ok = QInputDialog.getText(self, "新建脚本", "请输入脚本文件名（包含后缀，例如：myscript.ps1）:\n"
                                             ...
                                             f"注意：PsLauncher仅扫描 {DEFAULT_EXT} 这三种后缀, 如果后缀不正确, 创建后将无法在此处立即看见.", QLineEdit.Normal, "new_script.ps1")
        ...
        if new_ext and new_ext not in DEFAULT_EXT:
            reply = QMessageBox.question(self, "后缀警告", f"您输入的后缀 {new_ext} 不是 PsLauncher 支持的脚本类型（{DEFAULT_EXT}）。\n"
    ...
    def rename_selected_script(self):
        ...
        new_file_name, ok = QInputDialog.getText(self, "重命名脚本", f"请输入新的脚本文件名：\n"
                                                 f"当前文件名：{old_file_name}\n"
                                                 "注意：程序不会自动添加后缀，如果您不输入后缀，文件将没有扩展名。\n"
                                                 f"注意: PsLauncher仅扫描 {DEFAULT_EXT} 这三种后缀, 如果后缀不正确, 创建后将无法在此处立即看见.", QLineEdit.Normal, old_file_name)
    ...
    def copy_selected_script(self):
        ...
        new_file_name, ok = QInputDialog.getText(self, "复制脚本", f"请输入复制后的脚本文件名：\n"
                                                 ...
                                                 "注意：程序不会自动添加后缀，如果您不输入后缀，文件将没有扩展名。\n"
                                                 f"注意: PsLauncher仅扫描 {DEFAULT_EXT} 这三种后缀, 如果后缀不正确, 创建后将无法在此处立即看见.", QLineEdit.Normal, default_new_name)
        ...
        if item:
            ...
            if path:
                ...
                if os.path.isdir(path):
                    # 文件夹项：显示文件夹相关菜单
                    open_folder_action = QAction("📂 在资源管理器中打开", self)
                    ...
                    if ext in self.config.get('supported_extensions', DEFAULT_EXT):
                        ...
                        if ext in runnable_ext:
                            ...
                            if self.is_script_auto_run(path):
                                auto_run_action = QAction("🔄 取消启动时启动该脚本", self)
                            else:
                                auto_run_action = QAction("🔄 启动时启动该脚本", self)
            ...
            if os.path.isfile(script_path):
                ...
                if ext in runnable_ext:
                    try:
                        ...
                        print(f"自动运行脚本失败 '{script_path}': {e}")
                else:
                    print(f"自动运行跳过（不在可运行后缀列表中）: {script_path}")
            else:
                print(f"自动运行跳过（文件不存在）: {script_path}")
