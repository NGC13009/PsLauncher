...
```mermaid
flowchart TB
    A[准备 PsLauncher] --> B{选择部署方式}

    %% 方式一：Ollama
    B --> C[方式一: Ollama 方便]
    C --> C1[在 ollama-launcher 中配置好启动参数和环境变量]
    C1 --> C2[导出为 .ps1 或 .bat 脚本]
    C2 --> C3[PsLauncher 打开脚本所在目录]
    C3 --> C4[PsLauncher 启动脚本，管理 ollama 服务]

    %% 方式二：llama.cpp + LiteLLM
    B --> D[方式二: llama.cpp + LiteLLM 高性能高自由度]

    D --> D1[安装 uv]
    D1 --> D2[创建独立虚拟环境并安装 litellm]
    D2 --> D3[编写 config.yaml 配置本地和云端模型路由]
    D3 --> D4[编写一键启动脚本 ps1 或 bat]

    D4 --> D5[下载 GGUF 模型文件]
    D5 --> D6[安装 llama.cpp 并编写各模型的启动脚本]
    D6 --> D7[为每个本地模型分配不同端口]

    D7 --> E[PsLauncher 载入所有脚本目录]
    E --> F[通过 PsLauncher 启动]
    F --> G[LiteLLM 网关服务]
    F --> H[llama.cpp 后端 1]
    F --> I[llama.cpp 后端 N ...]

    G --> J[本地应用统一通过 OpenAI 兼容 API 调用]
    H --> J
    I --> J
```
